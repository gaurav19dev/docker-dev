...
then
    ...
    docker-compose -f docker-compose-local-prod.yml build  && docker-compose -f docker-compose-local-prod.yml up -d pmanager_http
    sudo chown 1000:1000 ../pmanager2/pgdata*
    docker exec -it pmanager.http bash -c "cp .env.dist .env && composer install --no-dev -o -a && php bin/console app:clean-ms-graph-data"
    ...
    docker-compose -f docker-compose-local-prod.yml up -d --build website_http sso_http worklog_service_http postcode_http staff_http portal_http sun_service_http queueworker_http
    ...
    docker exec -it sso.http bash -c "cp .env.dist .env && composer install --no-dev -o -a && php bin/console d:m:m -n && exit"

    printf "\n\n * * * * Entering in each containers for build * * * * \n\n"

    printf "\n\n * * * * Starting  build for apropos-worklog-service * * * * \n\n"
    docker exec -it worklog.http bash -c "cp .env.dist .env && composer install --no-dev -o -a && php bin/console d:m:m -n && exit"
    ...
    docker exec -it website.http bash -c "cp .env.dist .env && composer install --no-dev -o -a && cd client-area && ng analytics off && npm install && npm run build:prod && exit"

    printf "\n\n * * * * Starting build for apropos-postcode-service * * * * \n\n"
    docker exec -it postcode.http bash -c "cp .env.dist .env && composer install --no-dev -o -a && php bin/console d:m:m -n && php bin/console app:seed-all-data && exit"

    printf "\n\n * * * * Starting build for apropos-staff-site * * * * \n\n"
    docker exec -it staff.http bash -c "npm install && ng analytics off && npm run build:prod"

    printf "\n\n * * * * Starting build for apropos-portal-service * * * * \n\n"
    docker exec -it portal.http bash -c "composer install --no-dev -o -a && exit"

    printf "\n\n * * * * Starting build for queueworker-service * * * * \n\n"
    docker exec -it queueworker.http bash -c "composer install --no-dev -o -a && exit"
